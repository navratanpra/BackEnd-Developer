...
const updateBlogs = async function (req, res) {
      try {
        ...
        let paramsId = await blogModel.findById(blogId);
        if(paramsId.authorId!=authorId) return res.status(403).send({msg:`You are not allowed to update ${paramsId._id} blogId`})
        if (!paramsId) {
          return res
            .status(404)
            .send({ status: false, msg: "blogId is not correct" });
...
const deleteBlogs = async function (req, res) {
      try {
        ...
        if(blog.authorId!=authorId) return res.status(403).send({msg:`You are not allowed to update ${blog._id} blogId`})
        let deletedBlog = await blogModel.findOneAndUpdate(
          { _id: blogIdData },
          { isDeleted: true, deletedAt: new Date(), new: true }
        );
...
const deleteBlogsByQuery = async function (req, res) {
  try {
    ...
    if(matchedBlog.length==0) return res.status(404).send({msg:"no records matched"});
    ...
    if(particularIdBlog.length == 0) return res.status(404).send({msg:"no records matched"})
      
    await blogModel.updateMany({_id:{$in:particularIdBlog}},{$set:{isDeleted:true,deletedAt: new Date()}})
