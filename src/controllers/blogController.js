...
const createBlog = async function (req, res) {
  try {
    ...
    // we are checking the validation from line no-13 starting from title to line no upto 26 authorId.
    ...
    if (!authorId)
      return res.status(400).send({ status: false, msg: "AuthorId is a mandatory field" });
    ...
    if (!author)
      ...
      // we are creating a blog model.
    let savedBlog = await blogModel.create(blogData);
    return res.status(201).send({ status: true, msg: "data created", data: savedBlog });
  } catch (error) {
...
// =============================================================================================================================================
// we are accessing all data from the database mongoose compass

const getBlogs = async function (req, res) {
  try {
    let data = req.query;
    data.isDeleted = false; //this line already given in the project
    data.isPublished = true; // this line already given in the project
    // we are finding all the data when we put any data in the query param it will be shown
    // some condition isdeleted should be false and ispublished should be true
     
    let blog = await blogModel.find(data).count({ isDeleted: false, isPublished: true });
    // if the length is less then 1 and if it s showing 0 then the status will be false and will give a msg that blog not found.
...
const deleteBlogsByQuery = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length < 1)
      return res.status(400).send({ status: false, msg: "Please provide query params" });
    ...
    if (
      ...
      return res.status(400).send({ status: false, msg: "Query filters cant Be Blank" });
    ...
    return res.status(200).send({ status: true, data: "Blogs deleted Successfully" });
  } catch (error) {
    return res.status(500).send({ status: false, msg: error.message });
  }
};
